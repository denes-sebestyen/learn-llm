...
For each dimension, provide one integer score from 0 to 3, one or more evidence items, and a concise overall reason explaining the score. Each evidence item must contain a turnId, one or more segmentIds from that turn, an impact of either \"positive\" or \"negative\", and a concise comment explaining how those specific segments affect the judgment. Do not quote or reproduce evidence text. Write all learner-facing evaluation text (reason and evidence comment) in the same language as the learner's conversation. Do not recommend learning modules or decide product behavior.
